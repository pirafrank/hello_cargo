...
sudo chmod +a "user:macports allow search" "$HOME"
sudo chmod +a "user:macports allow search" "$HOME/pirafrank"
sudo chmod +a "user:macports allow search" "$HOME/pirafrank/ports"
...
# port test = lint + build + install + test + uninstall
(cd "$LOCAL_PORTS_PATH/$CATEGORY/$NAME" && sudo port test && echo "✅ Portfile test passed successfully.")
...
# Note: sudo port -vst install has trace logs and may not work in CI
echo "👉 Run 'sudo port -vs install $NAME' to test the new version."
